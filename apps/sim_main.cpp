...
int main(int argc, char** argv) {
    ...
    if (used_tiler) {
        ...
        // Print first Q tile output as a sanity check
        std::string first_O = "HBM.O[0:" + std::to_string(fa2_wl.Br) +
                              ",0:" + std::to_string(fa2_wl.d_head) + "]";
        if (ts.has(first_O)) {
            std::cout << "\nFA2 output tile O[Q0] (first 4 rows):\n";
            ts.print(first_O, fa2_wl.Br, fa2_wl.d_head, 4);
        }
