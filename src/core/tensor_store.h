...
//
// Units read and write named buffers here; the DMA unit copies them on
...
public:
    ...
    // Extract a row-slice: rows [row_start, row_start+nrows) from src_name.
    // src_name has full_cols columns (row-major). Result: nrows × full_cols.
    ...
    // Extract a column-slice: cols [col_start, col_start+ncols) from src_name.
    // src_name has full_cols columns. Result: full_rows × ncols.
    ...
    // Place a tile result into the correct position of a full output matrix.
    // tile_name is tile_rows × tile_cols; dst_name is full_rows × full_cols.
