...
struct VectorCoreConfig {
    uint32_t simd_width  = 64;   // elements per cycle
    uint32_t exp_latency = 4;    // extra cycles per SIMD group for exp/log
};

struct AccessCoreConfig {
    uint32_t bandwidth = 64;     // elements per cycle for transpose/scatter/gather
};
